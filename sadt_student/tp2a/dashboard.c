...
int main(int argc, char **argv)
{
    ...
    while (1) {
        ...
        if (are_all_frames_checked(must_see_frames, must_see_frames_len)) {
            ...
            // 300 + 200 + 100 => 600
            float left_road = (vision_state[0].road * 3 + vision_state[1].road * 2 + vision_state[2].road) / 6;
            float right_road = (vision_state[5].road * 3 + vision_state[4].road * 2 + vision_state[3].road) / 6;


            printf(CLEAR_STDOUT);
            printf("Coefs Left: %f, Right: %f\n\r", left_road, right_road);
            if (abs(left_road - right_road) < 15) {
